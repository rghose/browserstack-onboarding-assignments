...
def get_score(roll)
  ...
  while not uncounted_rolls.empty?
    ...
    if uncounted_rolls.count(1) >= 3
      3.times do
        uncounted_rolls.delete_at(uncounted_rolls.index(1))
      end
    ...
    while i >= 2
      if uncounted_rolls.count(i) >= 3
        3.times do
          uncounted_rolls.delete_at(uncounted_rolls.index(i))
        end
    ...
    count = uncounted_rolls.count(1)
    if count > 0
      uncounted_rolls.delete(1)
      result[:score] += (count*100)
    end
    count = uncounted_rolls.count(5)
    if count > 0
